...
- ✅ 阶段 2：导航系统 + 弹窗清道夫 + 恢复重试 + 咖啡厅任务 + 多任务批次 + 运行历史落盘
- ⬜ 阶段 3：批量移植日常/战斗/活动任务（邮件/任务奖励/PvP/委托/活动）
...
- 场景模板路径已定义但 PNG 未打包，需用户从 BAAS assets 搬运到 `app/src/main/assets/templates/` 后真实任务才能命中。搬运前跑咖啡厅任务会日志提示「未找到领取按钮」，属正常。
- 无 WorkManager 定时调度（阶段 4），目前需手动点「一键执行」。
